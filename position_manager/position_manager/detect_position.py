...
class detect_position(Node):
    def __init__(self):
        ...
        self.yellow_lower = [27, 95, 45]
        self.yellow_upper = [43, 255, 255]
    ...
    def get_pos(self, frame):
        ...
        lower_yellow = np.array(self.yellow_lower)
        upper_yellow = np.array(self.yellow_upper)
    ...
    def face_away(self):
        ...
        while(self.align_attempts < 3):
            ...
            cx = self.get_pos(frame)
            frame_cx = frame.shape[1] // 2
            if(cx is None or abs(cx - frame_cx) > 100):
                self.align_attempts = 0
                return
            else:
                self.send_move_command("turn_right")
