...
class PlayMusic_Request(metaclass=Metaclass_PlayMusic_Request):
    ...
    def __init__(self, **kwargs):
        ...
        self.num_loops = kwargs.get('num_loops', float())
    ...
    def num_loops(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'num_loops' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'num_loops' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
...
class Metaclass_PlayMusic(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'pupper_interfaces.srv.PlayMusic')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from pupper_interfaces.srv import _play_music
...
class PlayMusic(metaclass=Metaclass_PlayMusic):
    from pupper_interfaces.srv._play_music import PlayMusic_Request as Request
    from pupper_interfaces.srv._play_music import PlayMusic_Response as Response
